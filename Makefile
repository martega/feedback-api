...
test: node_modules $(CONFIG_FILE)
	@export NODE_ENV=test NODE_PATH=.:src; $(TEST_FILES) | xargs $(MOCHA) -R spec

test-nyan: node_modules $(CONFIG_FILE)
	@export NODE_ENV=test NODE_PATH=.:src; $(TEST_FILES) | xargs $(MOCHA) -R nyan

test-cov: node_modules
	@./bin/jscoverage src src-cov;
	@-export NODE_ENV=test NODE_PATH='.:src-cov'; $(TEST_FILES) | xargs $(MOCHA) -R html-cov > test_coverage.html;
	@rm -rf src-cov;
...
production: node_modules $(CONFIG_FILE)
	@NODE_ENV=production NODE_PATH=.:src node ./src/main.js 2>/dev/null &
